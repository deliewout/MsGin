...
void dae::InputManager::RemoveGamePadCommand(const std::unique_ptr<Command>& command)
{
	auto it = std::find_if(m_GamepadCommands.begin(), m_GamepadCommands.end(), [&](const std::unique_ptr<GamepadCommand>& cmd)
		{
			return cmd->Command.get() == command.get();
		});
	m_GamepadCommands.erase(it, m_GamepadCommands.end());
}

void dae::InputManager::RemoveKeyBoardCommand(const std::unique_ptr<Command>& command)  
{  
    auto it = std::find_if(m_KeyboardCommands.begin(), m_KeyboardCommands.end(), [&](const std::unique_ptr<KeyboardCommand>& cmd)  
    {  
        return cmd->Command.get() == command.get();  
    });  
